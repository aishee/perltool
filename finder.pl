...
system('cls');
system('Title Admin Control Panel Finder v1.0');
...
print"~ Enter site\n* ex: www.aishee.net or www.aishee.net/path\n->";
$site=<STDIN>;
chomp($site);

print"\n";
print"~ Enter the source code type:\n*ex: php or asp\n->";
print"\n";
$code=<STDIN>;
chomp($code);
...
if($code eq "asp")
{
  @path1=('admin/','administrator/','moderator/','webadmin/','adminarea/','bb-admin/','adminLogin/','admin_area/','panel-administracion/','instadmin/',
    ...
    if($response->content =~ /Username/ ||
       ...
       $response->content =~ /passwordlogin/ ||
       $response->content =~ /Usager/ ||
       $response->content =~ /usager/ ||
       $response->content =~ /passs/ ||
       $response->content =~ /loginnow/ ||
